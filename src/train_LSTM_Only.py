...
from src.data_processing import load_and_split_data, LaneChangeDataset

# ==== 你的 compute_ADE_FDE、evaluate_model、evaluate_and_visualize_single_trajectory 函数保持不变 ====
# 这里省略，直接复用你原来的
...
# ==== 训练函数 ====
def train_model(train_loader, val_loader, configs,learning_rate):
    ...
    param_info = (
        f"xr_LSTM_enc_in_{configs.enc_in}_d_model_{configs.d_model}_d_ff_{configs.d_ff}_ep_{num_epochs}_"
...
if __name__ == '__main__':
    ...
    save_path = train_model(train_loader, val_loader, configs,learning_rate)
    ...
    # 单轨迹可视化
    specific_file_path = os.path.join(data_folder, args.specific_file)
