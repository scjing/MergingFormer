...
def evaluate_model(model, test_loader, configs, save_path):
    """
    对测试集进行评估，计算 MSE、RMSE、MAE、MAPE、R2、ADE、FDE 等指标，
    同时对特定特征（speed 和 speedY）单独计算评估指标。
    """
    model.eval()
...
def evaluate_and_visualize_single_trajectory(csv_file_path, model, configs, save_path):
    """
    对单条轨迹数据进行评估，并绘制 Speed 和 SpeedY 的真实值与预测值对比图。
    """
    model.eval()
    device = configs.device
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_name', type=str,
                        default="xr_Transformer_enc_in_23_d_model_64_d_ff_1024_ep_10_e_layers_6_d_layers_6_embed_64_lr_0.0001",
                        help="Name of the model configuration folder.")
    parser.add_argument('--specific_file', type=str, default="car_55_data.csv",
    ...
    from models.Model_Transformer import Model_Transformer_Only
    model = Model_Transformer_Only(configs)
