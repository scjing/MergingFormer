...
def train():
    ...
    param_info = f"enc_in_{configs.enc_in}_d_model_{configs.d_model}_d_ff_{configs.d_ff}_ep_{num_epochs}_e_layers_{configs.e_layers}_d_layers_{configs.d_layers}_embed_{configs.embed}_lr_{learning_rate}"
        ...
        with torch.no_grad():
            with tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs} - Validation", unit="batch") as val_bar:
                for input_data, target_data, _, _ in val_bar:
                    ...
                    val_bar.set_postfix(val_loss=val_loss)
