...
def train():
    ...
    class Config:
        def __init__(self):
            ...
            self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # 使用 GPU 或 CPU
            self.seq_length = 10
    ...
    param_info = f"xr_LSTM_Transformer_enc_in_{configs.enc_in}_d_model_{configs.d_model}_d_ff_{configs.d_ff}_ep_{num_epochs}_" \
