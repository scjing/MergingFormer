...
def plot_distribution_comparison(true_values, predicted_values, title, save_path, filename):
    ...
    plt.hist(predicted_values, bins=bins, alpha=0.6, label=f'Predicted (μ={mu_pred:.2f}, σ={std_pred:.2f})', density=True)
...
def plot_distribution_histogram_custom(predicted_values, true_values, x_label, save_path, filename):
    ...
    ax.hist(predicted_values, label='Model speed distribution', bins=common_bins, alpha=0.5, density=True, color=pred_color)
    ax.hist(true_values, label='Actual speed distribution', bins=common_bins, alpha=0.5, density=True, color=true_color)
    ...
    ax.plot(x_values, pdf_pred, label='Model probability density', color=pred_color, linewidth=2)
    ax.plot(x_values, pdf_true, label='Actual probability density', color=true_color, linewidth=2, linestyle='--')
...
def evaluate_model(model, test_loader, configs, save_path, raw_target_csv=None):
    ...
    plot_distribution_histogram_custom(true_speed, pred_speed, 'SpeedX/(m/s)', save_path, 'speed_distribution_histogram_E.svg')
    plot_distribution_histogram_custom(true_speedY, pred_speedY, 'SpeedY/(m/s)', save_path, 'speedY_distribution_histogram_E.svg')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', type=str, default="../data/map2_data5",
    ...
    parser.add_argument('--specific_file', type=str, default="39_117_tracks.csv",
    ...
    evaluate_and_visualize_single_trajectory(os.path.join(args.data_folder, args.specific_file), model, configs, save_path)
